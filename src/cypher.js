...
module.exports = (hljs) => ({
  ...
  keywords: {
    keyword:
      "as asc ascending and assert by call case collect commit constraint create csv cypher delete desc descending detach distinct drop else end ends explain fieldterminator for foreach from headers in index is join limit load match merge nodetach not on optional order periodic profile remove return scan set skip start starts then union unique unwind use using when where with yield",
    literal: "true false null",
    $pattern: /\b[a-zA-Z0-9_\.]+\b/, // using word boundaries to match whole words
  },
  contains: [
